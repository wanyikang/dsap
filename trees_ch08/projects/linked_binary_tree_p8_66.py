...
class LinkedBinaryTree(BinaryTree):
    ...
    def _validate(self, p):
        ...
        if not p._path:
            raise ValueError('p must have a valid path')
        return p._node

    def _get_path(self, walk_node, node, path):
        """ Find the path for node `node` recursively."""
        path.append(walk_node)
        rlt = None
        if walk_node is node:
            rlt = path
        elif not walk_node._left and not walk_node._right:
            path.pop()  # pop walk_node if not find node
        else:
            left = right = None
            if walk_node._left:
                left = self._get_path(walk_node._left, node, path)
            if walk_node._right and not left:
                right = self._get_path(walk_node._right, node, path)
            if not left and not right:
                path.pop()  # pop walk_node if not find node
            rlt = left if left else right
        return rlt

    def _make_position(self, node):
        """ Return Position instance for given node (or None if no node)."""
        path = self._get_path(self._root, node, [])
    ...
    def parent(self, p):
        ...
        return self._make_position(p._path[-2])
